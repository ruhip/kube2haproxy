...
func (proxier *Proxier) handleEndpointsUpdate(endpoints *api.Endpoints) {
	...
	if len(endpoints.Subsets) == 0 {
		...
		glog.V(1).Infof("froad:so return")
		return
	}
...
func (proxier *Proxier) commitHaproxy() {
	if !proxier.master.IsSet() || proxier.skipCommit {
		...
		glog.V(1).Infof("froad:commitHaproxy")
...
func (proxier *Proxier) commitAndReloadHaproxyInit() error {
...
func (proxier *Proxier) commitAndReloadHaproxy() error {
	...
	proxier.lock.Lock()
	var templateData1 *proxy.ServiceUnit
	for key, value := range proxier.routeTable {
		glog.V(1).Infof("commitAndReloadHaproxy:key:%s,value:%v", key, value)
		templateData1 = value
		templateData := template.TemplateData{IPs: ips, RouteTable: proxier.routeTable, Rservice: templateData1}
		cfgBytes, err := template.RenderTemplateWithFuncs("haproxy_tpl", string(templateContent), templateData)
		glog.V(1).Infof("froad:cfgBytes:%s", string(cfgBytes))
		if err != nil {
			glog.V(1).Infof("cfgBytes:err:%v", err)
			//proxier.lock.Unlock()
			//return err
			continue
		}
		...
		// Reload HAProxy
		if err := proxier.haproxyInstance.Reload2(key, cfgBytes); err != nil {
			glog.V(1).Infof("Reload2:err:%v", err)
			//return err
		}
	}
	proxier.lock.Unlock()
